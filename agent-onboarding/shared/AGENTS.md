...
> **management-portal MCP**. Platform-agnostic: honored at repo root by Copilot and Cursor, surfaced to
> Claude Code / Roo via the `management-portal` skill, and shared by the in-app AI Chat Assistant / Thoth
> (same tool registry). One page. For full detail, load the skill / its `reference.md`.
>
> Canonical source: `agent-onboarding/DISCIPLINE.md` (v1.0.0). This file condenses it — never contradicts
> it. If behavior must change, change `DISCIPLINE.md` first, then regenerate this.
...
change real data.** You have the *tools*, not the database — the read tools are your only source of truth
and your only way to verify.
...
3. **ALIGN (board-first)** — for any non-trivial work, build an alignment **board** (mermaid + charts) and
...
- **MCP agent** (Claude Code / Copilot / Roo / Cursor): **the agent itself** reasons over read-tool output
  and writes clusters/relations/tasks via the primitive tools. **No DeepSeek.**
...
higher — raising it does not wait longer, it only loses the answer, because the MCP client throws away any
...
`list_channel_watchers(channel_id)` shows who is actually present, and the **coordinator** uses
...
*different people's* API keys but **not different sessions sharing one key** — the normal way this is used
(several sessions on one `pfk_live_…`, all carrying the same user id, so all passing; the only thing left
was `as_agent`, a string any caller can type). So a session **claims** the title and the server returns a
secret **exactly once**. Both tools are scope `team_chat.write`:
...
- `claim_coordinator_title(channel_id, title, as_agent?)` — call it **once**, as the coordinator, right
  ...
  **once and is never shown again**. **First-come:** a claim on an already-claimed title **fails** rather
  than taking over.
- `transfer_coordinator_title(channel_id, coordinator_token, to_agent?, title?, as_agent?)` — the
  ...
  current holder's token**: the creator/owner check is deliberately **not** accepted in its place, because
  on a shared key it admits every worker session.
...
directly — `DELETE FROM public.chat_channel_coordinators WHERE channel_id = '<uuid>';` then claiming again.
It is deliberately **not** a tool, because any recovery a coordinator could self-serve, a worker on the
same key could self-serve too. Call `transfer_coordinator_title` **before** ending a session others depend
on.
...
Re-arming first is safe: the previous watcher has already returned (that return is what invoked you), so two
never run at once; the cursor is unchanged, so no message is missed; and the call count is identical, so
there is no extra egress. Full procedure: the `team-chat-reachability` skill — the skill **teaches** it,
`/rearm-watch` is what a **human types** to check or re-arm by hand, and the `team-chat-watcher` sub-agent
is the one **you spawn**; only that last one actually makes you reachable.

**What will and will not tell you — it depends on the adapter, and usually nothing will.** Start with the
half that is *not* at risk: **no message is ever lost.** `await_my_turn` resumes from its `since` cursor,
so the moment you look again you are handed everything that arrived while you were not looking. An agent
that stops watching for ten minutes loses ten minutes of responsiveness and **zero messages** — this is a
**latency** problem, not a **delivery** problem. Nothing here is a queue, a retry, or a dead-letter path,
and anyone building one has misdiagnosed it.

The half that *is* at risk is that nobody notices. Only one adapter can tell you on its own: **Claude Code
with a `Stop` hook declared in `.claude/settings.json`** checks the roster at turn end and says so in
context. **Everywhere else, nothing interrupts you when you go `ABSENT`** — Copilot's hooks cover
`PreToolUse`/`PostToolUse` only and it has no turn-end event, and Cursor and Roo have no hook layer at
all. (A plugin-declared `Stop` hook fires fine — an earlier note here claiming otherwise was a probe
error, not a runtime limit.) And even where the alarm works it fires at *turn end*, so it shortens the blind gap
to one turn rather than removing it, and a session that has **ended** has no turn left to end — no alarm
will ever fire for it.

So on every platform the same two things carry the weight: **read
`list_channel_watchers(channel_id)` yourself** rather than assuming you are still on the roster, and
accept that the final backstop is a person — the human or the coordinator reading the roster. Treat a
`watching` row as the only evidence; your own belief that you re-armed is not evidence, because only
`await_my_turn` actually running writes the heartbeat. Per-adapter matrix, the measured hook facts, and
the honest limits: `../WATCH-LAYERS.md`.

## Where the detail lives

- **Skill** — `skills/management-portal/SKILL.md` (the operable how-to; triggers on portal work).
- **Reference** — `skills/management-portal/reference.md` (full discipline, principle→tool-sequence
  playbook, write→read verification map, board-first procedure).
- **Connection** — `mcp.config.md` (canonical MCP URL + per-platform config file).
- **Canon** — `../DISCIPLINE.md`, `../WRITE-READ-MAP.md`, `../BOARD-FIRST.md`, `../COVERAGE-AUDIT.md`.
