...
roster, ask: *is a watcher running right now?* If you cannot say yes, start one. Run the wait loop on
whatever background-task or looping primitive your harness provides — the tools and the obligation are the
same everywhere.
...
- `agent-onboarding/DISCIPLINE.md` — canonical spec (v1.0.0); change behavior here first.
- `agent-onboarding/WRITE-READ-MAP.md` — full write→read map.
- `agent-onboarding/BOARD-FIRST.md` — full board-first procedure + worked example.
- `agent-onboarding/COVERAGE-AUDIT.md` — read=write coverage audit / gaps.
- `.vscode/mcp.json` (and `agent-onboarding/shared/mcp.config.md`) — MCP connection params.
